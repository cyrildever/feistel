...
\renewcommand{\maketitlehookd}{%
    \begin{abstract}
        \noindent We define an obfuscation tool to secure data with an almost Format-Preserving Encryption process. Implementing a Feistel block cipher 
        with a round function using any robust hashing function, it provides you with a one-way tool to both encrypt and decrypt the data.
    \end{abstract}
}
...
We define the final round function $F$ as the hash of the previous addition, the result we XOR with the left part $L$ of the previous round to form the 
new basis for the next round where $L$ and the output of $F$ are switched.
\begin{equation}
    \label{eq:roundFunction}
    \begin{array}{rl}
        F: \omega \times \mathcal{K}  &\to \omega \\
                                (R, K)  &\mapsto \mathfrak{h} \left[ Add \Big(R), \mu(K, |R|) \Big) \right] \\
    \end{array}
\end{equation}
...
We finally define the full cipher $\mathfrak{F}$ that respects Figure \ref{fig:feistel} with $F_i = F(R, K)$ at round $i$ as follows:
\begin{equation}
    \label{eq:fullCipher}
    \begin{array}{rl}
...
On both environments our latest tests show no significant impact with a $10$ round cipher (a few dozens of nanoseconds at most). The results are in fact 
mostly impacted by the speed of the used hash function on the machine it is run (and obviously a little slower in the browser of an ordinary PC).
